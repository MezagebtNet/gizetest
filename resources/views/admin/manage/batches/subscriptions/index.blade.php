            ...
            <div class="card" id="main-card">
                <div class="card-header">
                    ...
                    @if ($batch)
                        ...
                        <a href="{{ route('admin.manage.batch.subscription.index', $gize_channel->id) }}"
                                            ...
                                            @for ($i = 1; $i <= $batch->max_period_no; $i++)
                                                <th scope="col">
                                                    <a href="javascript:void(0);" class="btn-edit-period"
                                                        ...
                                                        data-toggle="tooltip"
                                                        title="{{ $i }} | {{ $batch->subscription_periods[$i - 1]->name }} ({{ $batch->subscription_periods[$i - 1]->from_date }} - {{ $batch->subscription_periods[$i - 1]->to_date }})">{{ $i }}
                                    ...
                                    <tbody>
                                        @foreach ($subscriptions as $subscription)
                                            <tr id="subscriptionid{{ $subscription->id }}" style="height:90px;">
                                                <td class="align-middle"><input type="checkbox" name="ids"
                                                        class="checkBoxClass " value="{{ $subscription->id }}" /></td>
                                                ...
                                                <td class="align-middle">
                                                    <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                                                        ...
                                                        <div class="info" style="display: grid;">
                                                            ...
                                                            <span
                                                                class="text-sm">{{ $subscription->created_at }}</span>
                                                            <span
                                                ...
                                                @if (!$found)
                                                    ...
                                                    <td scope="col" id="{{ $subscription->id . '-' . $i }}"
                                                        ...
                                                        <button title="Add Payment Detail" type="button"
                                                            class="btn btn-sm btn-danger ml-2 btn_add_payment"
...
@section('js')
    ...
    <script>
        ...
        $('#periodForm').on('submit', function(e) {
            ...
            let url = "{{ route('admin.manage.batch.addperiod', ':gize_channel_id') }}";
        ...
        $('#periodEditForm').on('submit', function(e) {
            ...
            let url = "{{ route('admin.manage.batch.editperiod', ':gize_channel_id') }}";
            let gize_channel_id = "{{ $gize_channel->id }}";
            url = url.replace(':gize_channel_id', gize_channel_id);
        ...
        $(document).on('click', '.btn-approve', function(e) {
            ...
            let routeUrl = "{{ route('admin.manage.batch.subscription.approve', ':gize_channel_id') }}";
        ...
        $('#paymentForm').on('submit', function(e) {
            ...
            let url = "{{ route('admin.manage.batch.subscription.addpayment', ':gize_channel_id') }}";
        ...
        $('#paymentEditForm').on('submit', function(e) {
            ...
            let url = "{{ route('admin.manage.batch.subscription.editpayment', ':gize_channel_id') }}";
        ...
        function initialise_pmt_mnu() {
            ...
            $('.btn-remove-payment').on('click', function(e) {
                ...
                url =
                    "{{ route('admin.manage.batch.subscription.deletepayment', ['gize_channel_id' => ':gize_channel_id', 'batch_user_id' => ':batch_user_id', 'subscription_period_id' => ':subscription_period_id']) }}";
                url = url.replace(':gize_channel_id', gize_channel_id);
        ...
        $('#subscriptionForm').submit(function(e) {
            ...
            let gize_channel_id = "{{ $gize_channel->id }}";
            let url = "{{ route('admin.manage.batch.subscription.add', ':gize_channel_id') }}";
            url = url.replace(':gize_channel_id', gize_channel_id);
                    ...
                    if (response) {
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                ...
                                total_paid = "<strong>" + response.total_paid + " " +
                                    response.currency + "</strong>";
                                ...
                                //Total Paid Cell
                                $("td", rowNode).eq(cols_before_periods + max_period_no + 1)
                                    .attr("id",
                                        "total" + response.id);
