  <div title="{{ $archivevid->title }}" class="card mx-2 p-1 my-2  bg-gradient-dark" style="">
      <img class="card-img-top" src="{{ asset('storage/' . $archivevid->poster_image_url) }}"
          alt="{{ $archivevid->title }}">
  ...
  <div class="d-flex align-content-start flex-wrap mx-3 mb-3" style=" ">
      <div class="  align-self-center">
          {{-- <a href="{{ route('channel.landing', ['slug' => $archivevid->gizeChannel->slug]) }}"> --}}
          <img title="{{ $archivevid->gizeChannel->name }}" alt="{{ $archivevid->gizeChannel->name }}"
              class="border:1px solid gray;" src="{{ asset('storage/' . $archivevid->gizeChannel->logo_image_url) }}"
              width="40" />
          {{-- </a> --}}
      </div>
      <div class="d-flex flex-column bd-highlight ml-2">
          <h5 class="card-title text-dark ">{{ mb_strimwidth($archivevid->trainer, 0, 20, '...') }} -
              {{ mb_strimwidth($archivevid->title, 0, 10, '...') }}</h5>
